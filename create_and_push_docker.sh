...
if [[ ${1,,} == "cython" ]]; then 
    ...
    docker build -t eu.gcr.io/fluid-door-230710/upstride:py-`version`-tf2.4.1-gpu-cython-compiled -f dist_binary.dockerfile .
    docker push eu.gcr.io/fluid-door-230710/upstride:py-`cat version`-tf2.4.1-gpu-cython-compiled 
else
    echo "Building docker with Python Engine"
    docker build -t eu.gcr.io/fluid-door-230710/upstride:py-`cat version`-tf2.4.1-gpu .
    docker push eu.gcr.io/fluid-door-230710/upstride:py-`cat version`-tf2.4.1-gpu
fi
