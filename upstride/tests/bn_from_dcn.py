...
# THIS FILE IS from deep_complex_networks, and copied here for unit testing batch norm implementations
#
...
# changed regular keras imports to tensorflow.keras
import numpy as np
...
def sqrt_init(shape, dtype=None):
    value = (1 / np.sqrt(2)) * K.ones(shape) # changed K(keras backend) to np
    return value
...
def complex_standardization(input_centred, Vrr, Vii, Vri,
    ...
    s = K.sqrt(delta) # Determinant of square root matrix # changed np to K
    t = K.sqrt(tau + 2 * s) # changed np to K
...
class ComplexBatchNormalization(Layer):
    ...
    def call(self, inputs, training=None):
        ...
        if training in {0, False}:
            ...
            self.add_update(update_list) # no need to pass inputs in the latest version, its inferred automatically
