...
from . import utils
from .initializers import QInitializerConv
        ...
        if self.use_bias:
            if self.data_format == 'channels_first':
                if self.rank == 1:
                    ...
                    for i in range(self.ga_dimension):
                        bias = array_ops.reshape(self.biases[i], (1, self.filters, 1))
                        outputs[i] += bias
                else:
                    for i in range(self.ga_dimension):
                        outputs[i] = nn.bias_add(outputs[i], self.biases[i], data_format='NCHW')
            else:
                for i in range(self.ga_dimension):
                    outputs[i] = nn.bias_add(outputs[i], self.biases[i], data_format='NHWC')
...
class DepthwiseConv2D(Conv2D):
    ...
    def __init__(self,
        ...
        super(DepthwiseConv2D, self).__init__(
            ...
            **kwargs)
        self.depth_multiplier = depth_multiplier
        self.depthwise_initializer_type = depthwise_initializer
        self.depthwise_regularizer = regularizers.get(depthwise_regularizer)
        self.depthwise_constraint = constraints.get(depthwise_constraint)
        self.bias_initializer = initializers.get(bias_initializer)
