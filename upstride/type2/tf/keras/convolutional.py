...
class Conv(Layer):
    ...
    def call(self, inputs):
        ...
        outputs = utils.quaternion_mult(
            self._convolution_op, inputs, self.kernels)
                ...
                if self.rank == 1:
                    ...
                    for i in range(self.ga_dimension):
                        bias = array_ops.reshape(
                            self.biases[i], (1, self.filters, 1))
                        outputs[i] += bias
                else:
                    for i in range(self.ga_dimension):
                        outputs[i] = nn.bias_add(
                            outputs[i], self.biases[i], data_format='NCHW')
            else:
                for i in range(self.ga_dimension):
                    outputs[i] = nn.bias_add(
                        outputs[i], self.biases[i], data_format='NHWC')
