...
class Dense(Layer):
  ...
  def call(self, inputs):
    ...
    if rank is not None and rank > 2:
      ...
      outputs = quaternion_mult(lambda x,k: standard_ops.tensordot(x, k, [[rank - 1], [0]]), inputs, self.kernels)
      ...
      if K.is_sparse(inputs):
        outputs = quaternion_mult(sparse_ops.sparse_tensor_dense_matmul, inputs, self.kernels)
      else:
        outputs = quaternion_mult(gen_math_ops.mat_mul, inputs, self.kernels)
    if self.use_bias:
      for i in range(4):
