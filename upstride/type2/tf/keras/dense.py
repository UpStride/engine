...
class Masking(Layer):
    ...
    def call(self, inputs):
        ...
        outputs._keras_mask = array_ops.squeeze(
            boolean_mask, axis=-1)  # pylint: disable=protected-access
        return outputs
...
class Reshape(Layer):
    ...
    def compute_output_shape(self, input_shape):
        ...
        if None in input_shape[1:]:
            ...
            output_shape += tuple(s if s != -
                                  1 else None for s in self.target_shape)
        else:
            output_shape = [input_shape[0]]
    ...
    def call(self, inputs):
        ...
        if input_shape[1:].is_fully_defined():
            ...
            if batch_size:
                ...
                outputs = array_ops.reshape(
                    inputs, (array_ops.shape(inputs)[0], -1))
...
class Lambda(Layer):
    ...
    def compute_output_shape(self, input_shape):
        ...
        batch_size = nest.flatten(input_tensor_shape)[
            0][0] if input_shape else None
...
class Dense(Layer):
    ...
    def call(self, inputs):
        ...
        if rank is not None and rank > 2:
            ...
            outputs = quaternion_mult(lambda x, k: standard_ops.tensordot(
                x, k, [[rank - 1], [0]]), inputs, self.kernels)
            ...
            for i in range(4):
                inputs[i] = math_ops.cast(inputs[i], self._compute_dtype)
            if K.is_sparse(inputs):
                outputs = quaternion_mult(
                    sparse_ops.sparse_tensor_dense_matmul, inputs, self.kernels)
            else:
                outputs = quaternion_mult(
                    gen_math_ops.mat_mul, inputs, self.kernels)
        if self.use_bias:
            for i in range(4):
                outputs[i] = nn.bias_add(outputs[i], self.biases[i])
        if self.activation is not None:
            for i in range(4):
                outputs[i] = self.activation(
                    outputs[i])  # pylint: disable=not-callable
        return outputs
