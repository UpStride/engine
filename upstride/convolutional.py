...
# output_{i,j} = \sum_{lmn} conv2D(pointwise_mult(w^l/|w|, w^n/|w|), x^m) * epsilon_l e_l e_m e_n
  ...
  def build(self, input_shape):
    ...
    if input_channel % self.groups != 0:
      raise ValueError(
          ...
          '(full input shape is {}).'.format(self.groups, input_channel,
                                             input_shape))
    kernel_shape = (self.uptype.dimension,) + self.kernel_size + (input_channel // self.groups,
                                       self.filters)
    ...
    if self.use_bias:
      self.bias = self.add_weight(
          name='bias',
          shape=(self.uptype.dimension * self.filters,),
...
class Conv(Layer):
  ...
  def __init__(self, rank,
    ...
    self.ga_dimension = ga_dimension
  ...
  def build(self, input_shape):
    ...
    self._convolution_op = nn_ops.Convolution(
        input_shape,
        filter_shape=self.kernels[0].shape,
  ...
  def call(self, inputs):
    ...
    # compute the pointwise multiplication for all kernels
    pw_kernels = []
    for i in range(self.ga_dimension):
      pw_kernels.append([])
      for j in range(self.ga_dimension):
        pw_kernels[i].append(self.kernels[i] * self.kernels[j])
